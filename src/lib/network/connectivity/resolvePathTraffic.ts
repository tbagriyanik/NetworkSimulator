...
import { establishIpsecSa, encapsulateEsp, decapsulateEsp, type EspPacket } from '@/lib/network/ipsec';
...
export function resolvePathTraffic(
...
): { connectivity: ReturnType<typeof checkConnectivity>; qos?: QosScheduleResult; esp?: EspPacket; decapsulated?: { protocol: string; payload?: unknown } } {
  ...
  const sa = crypto?.setPeer && crypto.setTransformSet ? establishIpsecSa(crypto.setPeer, crypto.setTransformSet) : undefined;
  const esp = sa ? encapsulateEsp('ip', sa, packets) : undefined;
  const decapsulated = esp && sa ? decapsulateEsp(esp, sa) : undefined;
  return { connectivity, qos, esp, decapsulated };
}
