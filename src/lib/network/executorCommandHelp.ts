...
export const commandHelp: Record<string, Record<string, string[]>> = {
  ...
  'privileged': {
    ...
    ...pfx('show', ['access-lists', 'alias', 'ap', 'archive', 'arp', 'authentication', 'banner', 'boot', 'cdp', 'class-map', 'clock', 'controllers', 'debugging', 'diagnostic', 'dot11', 'environment', 'errdisable', 'etherchannel', 'event', 'flash', 'glbp', 'history', 'interface', 'interfaces', 'inventory', 'ip', 'ipv6', 'lldp', 'mac', 'memory', 'mls', 'monitor', 'nameif', 'netconf-yang', 'ntp', 'policy-map', 'port-security', 'processes', 'redundancy', 'route-map', 'running-config', 'sdm', 'sessions', 'snmp', 'spanning-tree', 'ssh', 'standby', 'startup-config', 'storm-control', 'system', 'udld', 'users', 'version', 'vlan', 'vtp', 'wireless', 'wlan']),
    'show event': ['manager'],
    'show event manager': ['applet'],
    ...
    'show ip igmp groups': [],
    'show ip inspect': ['config', 'interfaces'],
    'show ip cache': ['flow'],
    'show ip flow': ['export'],
    'show ip cache flow': [],
    'show flow': ['record', 'exporter', 'monitor'],
  ...
  'config': {
    '': ['access-list', 'alias', 'ap', 'archive', 'banner', 'cdp', 'channel', 'class-map', 'clock', 'crypto', 'default', 'do', 'dot11', 'dot1x', 'enable', 'end', 'errdisable', 'event', 'exit', 'hostname', 'interface', 'ip', 'ipv6', 'lldp', 'line', 'mac', 'macro', 'mls', 'monitor', 'netconf', 'netconf-yang', 'no', 'ntp', 'policy-map', 'route-map', 'router', 'same-security-traffic', 'sdm', 'security', 'service', 'snmp-server', 'spanning-tree', 'station-role', 'system', 'template', 'username', 'vlan', 'vtp', 'wlan'],
    'event': ['manager'],
    'event manager': ['applet'],
    'netconf': ['ssh'],
    ...pfx('route-map', ['<name>']),
    ...pfx('flow', ['record', 'exporter', 'monitor']),
    'flow record': ['<name>'],
    'flow exporter': ['<name>'],
    'flow monitor': ['<name>'],
    ...
    ...pfx('snmp-server', ['community', 'contact', 'group', 'host', 'location', 'user']),
    ...pfx('spanning-tree', ['backbonefast', 'loopguard', 'mode', 'portfast', 'uplinkfast', 'vlan']),
  ...
  'interface': {
    ...
    ...pfx('tunnel', ['destination', 'mode', 'protection', 'source']),
    'tunnel protection': ['ipsec'],
    'tunnel protection ipsec': ['profile'],
    ...pfx('ip', ['access-group', 'address', 'arp', 'dhcp', 'directed-broadcast', 'flow', 'helper-address', 'igmp', 'inspect', 'nat', 'pim', 'proxy-arp', 'verify']),
    'ip pim': ['dense-mode', 'sparse-dense-mode', 'sparse-mode'],
    'ip igmp': ['join-group', 'version'],
    'ip inspect': ['<name>'],
    'ip flow': ['egress', 'ingress', 'monitor'],
    'ip flow monitor': ['<name>'],
    'ip arp': ['inspection'],
  ...
  'router-config': {
    ...
    'area nssa': [],
    ...pfx('auto-summary', []),
    ...pfx('bgp', ['cluster-id', 'default', 'graceful-restart', 'router-id']),
    'bgp cluster-id': [],
    ...
    'eigrp router-id': [],
    ...pfx('maximum-paths', []),
    ...pfx('neighbor', ['allowas-in', 'as-override', 'default-originate', 'description', 'ebgp-multihop', 'maximum-prefix', 'med', 'next-hop-self', 'password', 'remote-as', 'remove-private-as', 'route-map', 'route-reflector-client', 'send-community', 'shutdown', 'soft-reconfiguration', 'timers', 'update-source', 'weight']),
    'neighbor allowas-in': [],
    ...
    'no bgp': ['cluster-id', 'default', 'graceful-restart'],
    'no bgp cluster-id': [],
...
export const commandDescriptions: Record<string, Record<string, string>> = {
  ...
  privileged: {
    ...
    'setup': 'Başlangıç kurulum diyaloğuna gir (Enter initial setup dialog) ',
    'test': 'Tanılama testlerini çalıştır (Run diagnostics) ',
    'more': 'Dosya veya yapılandırma içeriğini göster; örn. more system:running-config (Display contents of a file or configuration)',
    'disconnect': 'Ağ bağlantısını kes (Disconnect network connection) ',
    'resume': 'Askıya alınmış oturuma dön (Resume a suspended session) ',
    'suspend': 'Mevcut oturumu askıya al (Suspend current session) ',
    '?': 'Yardım göster (Display help)',
    'help': 'Yardım göster (Display help)',
  },
  config: {
    ...
    'default': 'Arayüzü varsayılan yapılandırmasına döndür; örn. default interface Gi0/1 (Reset an interface to its default configuration)',
    ...
    'template': 'Şablon yapılandırmasına gir (Enter template configuration) ',
  ...
  interface: {
    ...
    'priority-queue': 'Öncelikli kuyruk ayarları; çıkışta expedite kuyruğunu etkinleştir (Enable egress expedite queue)',
    'queue-set': 'Arayüz için egress kuyruk kümesini yapılandır (Configure the egress queue set)',
    'tx-queue': 'Arayüz çıkış (transmit) kuyruk numarasını ayarla (Set the transmit queue number)',
    ...
    'carrier-delay': 'Taşıyıcı (carrier) durum değişikliğinde arayüz geçiş gecikmesini saniye cinsinden ayarla (Set carrier delay in seconds)',
    'load-interval': 'İstatistik ölçüm aralığını ayarla; örn. load-interval 60 (Set input/output statistics interval, 30-600 s)',
  ...
  line: {
    ...
    'session-limit': 'Maksimum oturum sayısını ayarla (Set session limit) ',
    'autocommand': 'Otomatik komut ayarla (Set auto-command) ',
    'lockable': 'Hat kilitlemeyi etkinleştir (Enable line locking) ',
