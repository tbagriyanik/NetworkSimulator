...
export function executeCommand(
  ...
  const capabilityError = getCommandCapabilityError(commandName, state);
  if (capabilityError) {
    return processCommandResultExternal({
      success: false,
...
// Exported for testing the command registry completeness (every validated
// command must actually resolve to a handler, otherwise executeCommand
// silently no-ops with `{ success: true }`).
