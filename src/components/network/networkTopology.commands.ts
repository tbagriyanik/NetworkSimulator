  ...
  return [
    ...
    },
    {
      ...
      cmds: [
        ...
        ['template <name>', isTR ? 'Şablon (template) yapılandırması' : 'Configure template', '(config)#'],
        ['mac access-list extended <name>', isTR ? 'L2 MAC Erişim Listesi oluştur' : 'Create L2 MAC Access List', '(config)#'],
        ...
        ['flow record <name>', isTR ? 'Esnek NetFlow kayıt (record) tanımla' : 'Define Flexible NetFlow record', '(config)#'],
        ['flow exporter <name>', isTR ? 'NetFlow dışa aktarıcı (exporter) tanımla' : 'Define NetFlow exporter', '(config)#'],
        ['flow monitor <name>', isTR ? 'NetFlow izleyici (monitor) tanımla' : 'Define NetFlow monitor', '(config)#'],
        ...
        ['ip verify source', isTR ? 'IPSG (IP Source Guard) arayüz filtresi' : 'Enable IP Source Guard on interface', '(config-if)#'],
        ['switchport mode private-vlan {host|promiscuous}', isTR ? 'Private VLAN port modu' : 'Set Private VLAN port mode', '(config-if)#'],
        ['switchport backup interface <if>', isTR ? 'Flex-Links yedek arayüzü tanımla' : 'Define Flex-Links backup interface', '(config-if)#'],
        ['spanning-tree guard root', isTR ? 'STP Root Guard aç' : 'Enable STP Root Guard on interface', '(config-if)#'],
        ['spanning-tree bpdufilter enable', isTR ? 'STP BPDU Filter aç' : 'Enable STP BPDU Filter on interface', '(config-if)#'],
        ['router lisp', isTR ? 'LISP (Locator/ID Separation) yönlendirme aç' : 'Enable LISP routing', '(config)#'],
        ['database-mapping <eid> <rloc>', isTR ? 'LISP EID -> RLOC haritalama ekle' : 'Add LISP EID -> RLOC mapping', '(config)#'],
        ['control-plane', isTR ? 'CoPP (Control Plane Policing - CPU Koruma) moduna gir' : 'Enter CoPP CPU protection mode', '(config)#'],
        ['ip policy route-map <map>', isTR ? 'PBR (Policy-Based Routing) politikası uygula' : 'Apply PBR policy-map on interface', '(config-if)#'],
        ['eigrp stub {connected|summary}', isTR ? 'EIGRP Stub yönlendirici modu' : 'Configure EIGRP Stub mode', '(config-router)#'],
      ]
    },
