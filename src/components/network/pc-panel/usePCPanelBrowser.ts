    ...
    if (resolvedTargetIp === '1.1.1.1' || resolvedTargetIp === '1.0.0.1' || resolvedTargetIp === '8.8.8.8' || resolvedTargetIp === '8.8.4.4' || (cloudTarget && !httpServer)) {
      ...
      setHttpAppContent(`
        <main style="padding:32px;font-family:'Inria Sans',sans-serif;text-align:center;">
          ...
          <div style="border:1px solid var(--color-secondary-300);border-radius:24px;padding:10px 20px;max-width:320px;margin:0 auto 20px;font-size:13px;color:var(--color-secondary-700);">ğŸ” ${language === 'tr' ? 'Arama yapın veya URL girin' : 'Search or type URL'}</div>
