  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent
        ...
        } sm:max-w-2xl rounded-2xl md:rounded-3xl shadow-2xl max-h-[calc(100dvh-1rem)] sm:max-h-[calc(100dvh-2.5rem)] flex flex-col p-4 sm:p-6 overflow-hidden`}
        onEscapeKeyDown={isLoading ? undefined : () => handleClose()}
        onPointerDownOutside={isLoading ? undefined : () => handleClose()}
      >
        {/* Header */}
        <DialogHeader className="shrink-0 space-y-1 pb-1">
          <div className="flex items-center justify-between">
            <DialogTitle className="text-xl font-bold flex items-center gap-2.5">
              ...
              <span>{isTr ? 'Otomatik Topoloji Üretici' : 'Automatic Topology Generator'}</span>
            </DialogTitle>
            <span className="text-[11px] font-semibold px-2 py-0.5 rounded-full bg-purple-500/10 text-purple-400 border border-purple-500/20">
              {SCENARIOS.length} {isTr ? 'Örnek Mimarisi' : 'Architectures'}
            </span>
          </div>
          <DialogDescription className={`text-xs ${isDark ? 'text-secondary-400' : 'text-secondary-500'}`}>
            {isTr
              ? 'Standart topolojiler, veri merkezleri, kampüs ağları ve güvenlik senaryolarını tek tıkla otomatik oluşturun.'
              : 'Instantly generate standard topologies, data centers, enterprise campus and security architectures with one click.'}
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="shrink-0 space-y-2 py-2">
          ...
          <div className="relative">
            ...
            <Input
              ...
              placeholder={isTr ? 'Örnek veya mimari ara (Örn: Spine-Leaf, Full Mesh, OSPF, BGP)...' : 'Search scenario (e.g. Spine-Leaf, Full Mesh, OSPF, BGP)...'}
            ...
            {searchQuery && (
              <button
                ...
                className={`absolute right-3 top-1/2 -translate-y-1/2 text-xs font-semibold ${isDark ? 'text-secondary-400 hover:text-white' : 'text-secondary-500 hover:text-secondary-900'}`}
              >
                ✕
              </button>
            )}
        ...
        {/* Scenarios Grid / List */}
        <div className="flex-1 overflow-y-auto min-h-[220px] max-h-[360px] pr-1 space-y-3 custom-scrollbar">
          {filteredScenarios.length > 0 ? (
              ...
              {filteredScenarios.map(s => {
                ...
                return (
                  <button
                    ...
                    className={`flex flex-col justify-between p-3 rounded-xl text-left transition-all duration-150 border relative group ${
                      isSelected
                        ? isDark
            ...
            <div className={`text-center py-12 text-sm flex flex-col items-center justify-center gap-2 ${isDark ? 'text-secondary-500' : 'text-secondary-400'}`}>
        ...
        {/* Selected Scenario Preview & Customization (PC Count) */}
        <div className={`shrink-0 pt-3 border-t ${isDark ? 'border-secondary-800' : 'border-secondary-200'} space-y-3`}>
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5">
            {/* Selected description pill */}
            <div className="flex items-center gap-2 min-w-0 flex-1">
              <div className={`p-1.5 rounded-md shrink-0 ${isDark ? 'bg-purple-950/40 text-purple-400 border border-purple-500/20' : 'bg-purple-100 text-purple-700'}`}>
            ...
            {selectedDef.showPcCount && (
              <div className="flex items-center gap-1.5 shrink-0">
                <Label className="text-[11px] font-bold shrink-0">{isTr ? 'Uç Cihaz:' : 'Clients:'}</Label>
          ...
          <div className="flex items-center justify-end gap-2 pt-1">
            ...
            <Button
              ...
              disabled={isLoading}
              className="bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-700 hover:to-indigo-700 text-white font-bold text-xs h-9 px-5 rounded-xl shadow-md shadow-purple-600/30 transition-all active:scale-[0.98]"
            >
              {isLoading ? (
                <>
