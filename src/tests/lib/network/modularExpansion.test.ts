...
describe('Modular Expansion Engine (Slots & WIC Modules)', () => {
  ...
  it('should install WIC-2T module and add Serial ports dynamically', () => {
    ...
    expect(res.newPorts[0].id).toBe('Serial1/0/0');
    expect(res.newPorts[1].id).toBe('Serial1/0/1');
    expect(res.newPorts[0].type).toBe('serial');

    expect(res.updatedSwitchState?.ports['Serial1/0/0']).toBeDefined();
    expect(res.updatedSwitchState?.ports['Serial1/0/0'].type).toBe('serial');
  });
  ...
  it('should install HWIC-4ESW 4-port switch module', () => {
    ...
    expect(res.newPorts[0].id).toBe('FastEthernet2/0/0');
    expect(res.newPorts[3].id).toBe('FastEthernet2/0/3');
  });
  ...
  it('should remove module and clean up attached connections and ports', () => {
    ...
    const connections: CanvasConnection[] = [
      {
        ...
        sourcePort: 'Serial1/0/0',
        targetDeviceId: 'router-2',
        targetPort: 'Serial1/0/0',
        cableType: 'serial',
        active: true,
      },
      {
    ...
    expect(removeRes.updatedDevice.ports.some((p) => p.id.includes('Serial1/0/'))).toBe(false);
