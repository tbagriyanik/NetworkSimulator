...
| Özellik | Güncel kapsam ve sınır |
|---|---|
| **Gelişmiş Port/Protokol Ping & BGP Oturum Sıfırlama** | `ping tcp <host> [port <p>]` ve `ping udp <host> [port <p>]` ile TCP/UDP port seviyesi erişilebilirlik ve SYN/response testi. `clear ip bgp *` / `clear ip bgp <neighbor>` ile BGP oturumlarını ve öğrenilen dinamik rotaları canlı sıfırlama, `soft` yeniden başlatma. `show ip bgp <prefix>` ile subnet bazlı detaylı BGP path, AS-Path, localpref ve weight analizi. |
| **Express Forwarding (CEF) & Port İstatistikleri** | `show ip cef [prefix] [detail]` ile Layer 3 cihazlarda CEF forwarding FIB tablosu gösterimi. `show interfaces counters [errors]` ile port bazında RX/TX paket, byte, drop ve hata sayaçları tablosu. `show ip dhcp snooping statistics` ile DHCP Snooping arayüz paket iletim/drop istatistikleri. `write erase` komut desteği. |
| **Dinamik Kablosuz Sinyal & Çubuk Algoritması** | Akıllı mesafe ve sinyal gücü hesaplaması (`<150px`: 5 yeşil, `<280px`: 4 yeşil, `<400px`: 3 yeşil, `<500px`: 2 yeşil, `<600px`: 1 yeşil çubuk; `≥600px`: 0 çubuk - 4 gri çizgi ve bağlantı kopması). Kablosuz bağlantı hattının sinyal seviyesiyle canlı senkronizasyonu. |
| **Yazıcı Web Arayüzü & Mobil Canlı SSID Bağlantısı** | PC web tarayıcısından HTTP port 80 üzerinden erişilebilen HTML **Yazıcı Web Paneli** (`printerWebPanel.ts`). Mobil cihazlarda canlı AP/Router/Switch SSID tarama ve seçim anında otomatik DHCP IP tahsisi. |
| **Çok Modlu CLI Alias & Dinamik Oturum Yönetimi** | `alias exec`, `alias configure`, `alias interface`, `alias line` ile mod bazlı alias tanımlama, silme (`no alias`), CLI parser çözümlemesi ve `show alias [mode]` listelemesi tamamlandı. `setup` komutu ile interaktif sistem kurulum sihirbazı, `suspend`, `resume [id]`, `disconnect [id]`, `show sessions` ile arka plan uzak oturum yaşam döngüsü ve `test memory/interfaces/cable-diagnostics/vlan` ile dinamik donanım ve kablo tanılama motoru entegre edildi. |
...
| Özellik | Güncel kapsam ve sınır |
|---|---|
| **EIGRP Arayüz Komutları** | `ip bandwidth-percent eigrp <as> <percent>` ve `no ip bandwidth-percent eigrp <as>` desteği arayüz düzeyine eklendi. `ip summary-address eigrp <as> <ip> <maskcaps> [<distance>]` referans kalıbı ve özet rota oluşturması sağlandı. |
...
| **Dinamik Müzik ve Ses Sentetörü** | Embedded Python ile Web Audio API tabanlı nota, polifonik akor (`play_chord`), melodi (`play_melody`), tonlama (`play_scale`), akor ilerlemesi (`play_progression`), arpej (`play_arpeggio`), ADSR zarfı, ses efektleri (`play_sfx`), nota teorisi yardımcıları, diske WAV kaydetme (`save_wav`) ve sanal diskten WAV okuyup çalma (`play_wav`) (`pcAudioTheory.ts`, `pcAudioRender.ts`, `pcAudioPlayer.ts`, `pcPythonAudioModule.ts`). |
...
| **Topoloji Anlık Görüntü & Geri Yükleme Türkçe UI** | Snapshot/Rollback terimleri Türkçe arayüze dönüştürüldü: "Topoloji Anlık Görüntü & Geri Yükleme Yöneticisi", "Geri Yükleme Noktası (Checkpoint)", "Geri Yükle". İçe aktarılan JSON dosyalarının şema ve cihaz yapısı `validateTopologyCheckpoint` ile sıkı doğrulanır, eksik veya uyumsuz JSON'larda aktarım engellenerek detaylı hata uyarısı verilir (`SnapshotManagerModal.tsx`, `snapshotManager.ts`). |
...
| **"Neden Ping Gitmiyor?" Kök Neden Teşhisi** | L1 fiziksel link/shutdown, L2 VLAN/Trunk/Native VLAN uyumsuzlukları, L3 Subnet & Default Gateway eksikliklerini otomatik tarayıp çözüm önerileri sunan teşhis motoru ve modalı (`networkTroubleshooter.ts`, `NetworkDiagnosticsModal.tsx`). |
...
| **Kablo Etiketleri Aç/Kapa** | Kablolar üzerindeki arayüz isimlerini (Gi0/0, Fa0/1) tuvalde doğrudan açıp kapatabilme seçeneği (`ViewVisibilityMenu.tsx`). |
