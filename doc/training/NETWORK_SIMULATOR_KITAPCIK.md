...
- **🎵 Web Audio API Tabanlı Dinamik Müzik & Ses Sentetörü (v5.4.0):** Gömülü Python (`audio` / `music` / `sound` / `synth` / `winsound`) üzerinden nota frekans çevrimi (C0-B8), polifonik akorlar (`play_chord`), hazır melodiler (`play_melody`), tonlama (`play_scale`), akor ilerlemesi (`play_progression`), arpej (`play_arpeggio`), PolyBLEP bant sınırlı dalga formları, ses efektleri (`coin`, `laser`, `jump`, `explosion`, `powerup`) ve ADSR zarfı sentezli seslerin ham WAV formatında sanal diske (`C:\*.wav`) aktarılması eklendi (`pcAudioTheory.ts`, `pcAudioRender.ts`, `pcAudioPlayer.ts`, `pcPythonAudioModule.ts`).
...
- **🛡️ Katman 2 MAC Erişim Listesi & Port Filtreleme (v5.3.0):** `mac access-list extended <name>` ve `mac access-group <name> in` kuralları ile Ethernet çerçeve seviyesinde filtreleme ve düşürme (`commonForwardingEngine.ts`).
...
- **🌐 BGP / MP-BGP & MPLS LDP Motorları:** Autonomous System (AS) yönlendirmesi, eBGP/iBGP durum makineleri (`Established`), VRF/RD/RT taşıma, LDP oturumları, LFIB etiket takası ve PHP `Implicit-Null` desteği (`bgpEngine.ts`, `mplsLdpEngine.ts`).
- **🩺 "Neden Ping Gitmiyor?" Kök Neden Teşhisi:** L1 fiziksel link/shutdown, L2 VLAN/Trunk/Native VLAN uyumsuzlukları, L3 Subnet & Default Gateway eksikliklerini otomatik tarayıp çözüm önerileri sunan teşhis motoru (`networkTroubleshooter.ts`, `NetworkDiagnosticsModal.tsx`).
...
| **Dynamic Audio & Music Synthesizer**: Web Audio API tone synthesis, polyphonic chords (`play_chord`), melodies (`play_melody`), scales (`play_scale`), chord progressions (`play_progression`), arpeggios (`play_arpeggio`), ADSR envelopes, sound effects (`play_sfx`), note-theory helpers, disk WAV export (`save_wav`) and WAV playback (`play_wav`). | **Dinamik Ses ve Müzik Sentetörü**: Web Audio API tabanlı nota sentezi, polifonik akorlar (`play_chord`), melodiler (`play_melody`), tonlamalar (`play_scale`), akor ilerlemeleri (`play_progression`), arpejler (`play_arpeggio`), ADSR zarfı, ses efektleri (`play_sfx`), nota teorisi yardımcıları, diske WAV kaydetme (`save_wav`) ve sanal diskten WAV çalma (`play_wav`). |
