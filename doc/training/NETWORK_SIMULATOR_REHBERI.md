...
- **Dinamik Müzik ve Ses Sentetörü (`audio` / `music` / `sound` / `synth` / `winsound`):** Web Audio API tabanlı nota (`play_note`, `note_to_freq`), polifonik akor (`play_chord`), melodi (`play_melody`), tonlama (`play_scale`), akor ilerlemesi (`play_progression`), arpej (`play_arpeggio`), ADSR zarfı (`Synth` / `Track`), ses efektleri (`play_sfx`), nota teorisi yardımcıları (`scale_notes`, `chord_notes`, `list_scales`, `list_chords`), WAV dosyası kaydetme (`save_wav`) ve sanal diskten WAV çalma (`play_wav`). Tüm fonksiyonlar hem konumsal hem adlandırılmış parametre (`keyword argument`) kabul eder.
